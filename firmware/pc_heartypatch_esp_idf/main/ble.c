    ...
    case ESP_GATTS_READ_EVT: 
		{
			...
			rsp.attr_value.value[0] = (0x11)| (ble_data_update.flag); 			//flag:: setas - 16 bit HR, 16 bit energy expended, 16 bit RR 
			rsp.attr_value.value[1] = ble_data_update.hr;						// LSO of HR
...
static void gatts_profile_temp_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.value[2] = (ble_data_update.temperature);
...
void kalam_ble_Init(void)
{
    ...
    //update_ble_atts(update_hr,9);
	//TaskHandle_t ble_profile_task;
    //xTaskCreate(&ble_profile_event_task, "ble_profile_event_task", 4096, NULL, 3 &ble_profile_task);
}	